...
class Server{
	function __construct($config=array()){
		...
		require_once(__DIR__.'/../ThirdParty/Oauth2/src/OAuth2/Autoloader.php');	//oauth library
		//require_once(__DIR__.'/../ThirdParty/Oauth2/src/OAuth2/Storage/Pdo.php');	//oauth library
		$DBConfig = new \Config\Database();
		$config = $DBConfig->db;
		\OAuth2\Autoloader::register();
		//\ThirdParty\Oauth2\src\OAuth2\Autoloader::register();
		$this->storage = new Pdo(array('dsn' => $config["DSN"], 'username' => $config["username"], 'password' => $config["password"]));
		$this->server = new Host($this->storage, array('allow_implicit' => true));
